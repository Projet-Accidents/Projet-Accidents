...
df_usagers = load_csv_data(r"C:\Users\macha\Desktop\Test_py\all_data_usagers_2019_2023.csv")
df_caracteristiques = load_csv_data(r"C:\Users\macha\Desktop\Test_py\all_data_carac_2019_2023.csv")
df_lieux = load_csv_data(r"C:\Users\macha\Desktop\Test_py\all_data_lieux_2019_2023.csv")
df_vehicules = load_csv_data(r"C:\Users\macha\Desktop\Test_py\all_data_vehicules_2019_2023.csv")
df_total_final = load_csv_data(r"C:\Users\macha\Desktop\Test_py\df_total_final.csv")
df_machine_learning = load_csv_data(r"C:\Users\macha\Desktop\Test_py\df_machine_learning.csv")
df_usagfinal = load_csv_data(r"C:\Users\macha\Desktop\Test_py\df_usagfinal.csv")
df_lieux_final = load_csv_data(r"C:\Users\macha\Desktop\Test_py\df_lieux_final.csv")
df_carac_lieux = load_csv_data(r"C:\Users\macha\Desktop\Test_py\df_carac_lieux.csv")
df_merged_usag_veh_final = load_csv_data(r"C:\Users\macha\Desktop\Test_py\df_merged_usag_veh_final.csv")
df = load_csv_data(r"C:\Users\macha\Desktop\Test_py\df_analyse_exploratoire.csv")
df_1 = load_csv_data(r"C:\Users\macha\Desktop\Test_py\df_1.csv")
...
st.sidebar.title("Prédiction de la gravité des accidents de la circulation routière en France")
...
if st.session_state.show_modelisation:
    Dataframe_machine_learning_checkbox = st.sidebar.checkbox("Dataframe Machine learning")
...
# Elaboration dataset      
    ...
    if elaboration_dataset_checkbox:
        ...
        st.write("Beaucoup de variables ont été encodées lors de la fusion des 4 dataframes, nous pourrons donc supprimer les colonnes 'variable_non_déterminé' pour les exclure sans perdre les autres informations des accidents, usagers, véhicules")
...
######################################### Page Datavisualisation et ses sous rubriques ################################
    ...
    if not any([distribution_gravité_checkbox,conditions_accident_checkbox ,localisation_checkbox ,usagers_checkbox,vehicules_checkbox ]):
...
# Analyse de la variable gravité
    if distribution_gravité_checkbox:
        st.subheader('Analyse de la gravité des accidents de circulation (2019-2023)')
...
# Conditions accident
    ...
    if usagers_checkbox:
        ...
        if "Système de sécurité" in variables_usagers:
            ...
            if 'df' not in locals():
                st.warning("Le DataFrame `df` n'est pas encore défini. Veuillez charger les données.")
            else:
                # --- 1. Préparation Initiale ---
                data_secu = df[['grav_max', 'secu1']].copy()

                secu_mapping = {
                    -1: 'Autres_Secu', 0: 'Pas_Equipement', 1: 'Ceinture', 2: 'Casque',
                    3: 'Autres_Secu', 4: 'Autres_Secu', 5: 'Autres_Secu', 6: 'Autres_Secu',
                    7: 'Autres_Secu', 8: 'Autres_Secu', 9: 'Autres_Secu'
                }

                data_secu['secu1'] = pd.to_numeric(data_secu['secu1'], errors='coerce')
                data_secu['secu1'] = data_secu['secu1'].map(secu_mapping)
                data_secu.dropna(subset=['secu1'], inplace=True)

                ordered_secu = ['Ceinture', 'Casque', 'Pas_Equipement', 'Autres_Secu']
                ordered_gravite = ['Blessé léger', 'Blessé hospitalisé', 'Décès']
                data_secu = data_secu[data_secu['secu1'].isin(ordered_secu)]

                if not data_secu.empty:
                    # --- 2. Agrégation ---
                    agg_data = data_secu.groupby(['secu1', 'grav_max']).size().reset_index(name='Nombre_Usagers')

                    # --- 3. Totaux par catégorie ---
                    total_par_secu = agg_data.groupby('secu1')['Nombre_Usagers'].sum().reset_index()
                    total_par_secu.rename(columns={'Nombre_Usagers': 'Total_Usagers_SECU'}, inplace=True)

                    # --- 4. Pourcentages ---
                    agg_data = pd.merge(agg_data, total_par_secu, on='secu1', how='left')
                    agg_data['Pourcentage'] = agg_data.apply(
                        lambda row: (row['Nombre_Usagers'] / row['Total_Usagers_SECU']) * 100 if row['Total_Usagers_SECU'] > 0 else 0,
                        axis=1
                    )
                    agg_data['Pourcentage'].fillna(0, inplace=True)
                    ...
                    # --- 5. Pivot Tables ---
                    heatmap_percentages = agg_data.pivot_table(
                        index='grav_max', columns='secu1', values='Pourcentage', fill_value=0
                    ).reindex(index=ordered_gravite, columns=ordered_secu, fill_value=0)

                    heatmap_counts = agg_data.pivot_table(
                        index='grav_max', columns='secu1', values='Nombre_Usagers', fill_value=0
                    ).reindex(index=ordered_gravite, columns=ordered_secu, fill_value=0)

                    # --- 6. Customdata ---
                    total_counts_ordered = total_par_secu.set_index('secu1').reindex(ordered_secu)['Total_Usagers_SECU'].fillna(0).values
                    num_gravity_levels = len(ordered_gravite)
                    totals_array = np.tile(total_counts_ordered, (num_gravity_levels, 1))
                    specific_counts_array = heatmap_counts.values
                    customdata_combined = np.stack([specific_counts_array, totals_array], axis=-1)
                    ...
                    fig = px.imshow(
                        heatmap_percentages,
                        labels=dict(x="Équipement de Sécurité", y="Gravité", color="Pourcentage d'Usagers (%)"),
                        title="Répartition (en %) de la Gravité par Équipement de Sécurité",
                        text_auto=True,
                        aspect="auto",
                        color_continuous_scale='YlGnBu'
                    )
                    ...
                    fig.update_traces(
                        ...
                        hovertemplate=(
                            "<b>Équipement:</b> %{x}<br>" +
                            "<b>Gravité:</b> %{y}<br>" +
                            "<b>Pourcentage d'Usagers:</b> %{z:.2f}%<br>" +
                            "<b>Nb Usagers (Spécifique):</b> %{customdata[0]:,.0f}<br>" +
                            "<b>Total Usagers (Équipement):</b> %{customdata[1]:,.0f}<extra></extra>"
                        )
                    )

                    fig.update_layout(
                        xaxis=dict(title='Équipement de Sécurité', type='category'),
                        yaxis=dict(title='Gravité'),
                        title_font_size=18,
                        xaxis_title_font_size=14,
                        yaxis_title_font_size=14,
                        coloraxis_colorbar=dict(title="Pourcentage d'Usagers (%)"),
                        xaxis_tickangle=0
                    )
                    ...
                    st.info("Le DataFrame filtré est vide, aucun graphique à afficher.")
    ...
    if vehicules_checkbox:
        ...
        if "Type de véhicule" in variables_vehicule:
            ...
            if 'df_1' not in locals():
                st.warning("Veuillez charger les données dans un DataFrame nommé `df`.")
            else:
                data_catv = df_1[['Num_Acc', 'grav_max', 'new_catv']].copy()

            # Gérer les NaN et vérifier si le DataFrame est vide
                data_catv.dropna(subset=['new_catv'], inplace=True)

                if not data_catv.empty:
                    # Agrégation
                    agg_data = data_catv.groupby(['grav_max', 'new_catv'])['Num_Acc'].nunique().reset_index()
                    agg_data.rename(columns={'Num_Acc': 'Nombre_Accidents_Uniques'}, inplace=True)

                    total_par_catv = data_catv.groupby('new_catv')['Num_Acc'].nunique().reset_index()
                    total_par_catv.rename(columns={'Num_Acc': 'Total_Accidents_CatV'}, inplace=True)

                    agg_data = pd.merge(agg_data, total_par_catv, on='new_catv', how='left')
                    agg_data['Pourcentage'] = agg_data.apply(
                        lambda row: (row['Nombre_Accidents_Uniques'] / row['Total_Accidents_CatV']) * 100
                        if row['Total_Accidents_CatV'] > 0 else 0,
                        axis=1
                    )
                    agg_data['Pourcentage'].fillna(0, inplace=True)

                    # Filtrage
                    agg_data = agg_data[agg_data['new_catv'] != 'Catv_NR']
                    total_counts = total_par_catv[total_par_catv['new_catv'] != 'Catv_NR']
                    sorted_catv_labels = total_counts.sort_values(by='Total_Accidents_CatV', ascending=False)['new_catv'].tolist()
                    grav_order_xaxis = ['Blessé léger', 'Blessé hospitalisé', 'Décès']

                    # Pivot
                    heatmap_percentages = agg_data.pivot_table(
                        index='new_catv', columns='grav_max', values='Pourcentage', fill_value=0
                    ).reindex(columns=grav_order_xaxis).reindex(index=sorted_catv_labels)

                    heatmap_counts = agg_data.pivot_table(
                        index='new_catv', columns='grav_max', values='Nombre_Accidents_Uniques', fill_value=0
                    ).reindex(columns=grav_order_xaxis).reindex(index=sorted_catv_labels)

                    # Customdata pour hover
                    total_counts_ordered = total_counts.set_index('new_catv')['Total_Accidents_CatV'].reindex(sorted_catv_labels).values
                    num_gravity_levels = len(grav_order_xaxis)
                    totals_array = np.tile(total_counts_ordered, (num_gravity_levels, 1)).T
                    specific_counts_array = heatmap_counts.values
                    customdata_combined = np.stack([specific_counts_array, totals_array], axis=-1)
                    ...
                    fig = px.imshow(
                        heatmap_percentages,
                        labels=dict(x="Gravité", y="Type de Véhicule (Trié)", color="Pourcentage (%)"),
                        title="Répartition (en %) de la Gravité des Accidents par Type de Véhicule",
                        text_auto=True,
                        aspect="auto",
                        color_continuous_scale='YlGnBu'
                    )
                    ...
                    fig.update_traces(
                        ...
                        hovertemplate=(
                            "<b>Gravité:</b> %{x}<br>" +
                            "<b>Type de Véhicule:</b> %{y}<br>" +
                            "<b>Pourcentage:</b> %{z:.2f}%<br>" +
                            "<b>Nb Accidents (Gravité):</b> %{customdata[0]:,.0f}<br>" +
                            "<b>Total Accidents (Type de Véhicule):</b> %{customdata[1]:,.0f}<extra></extra>"
                        )
                    )

                    fig.update_layout(
                        xaxis=dict(title='Gravité', type='category'),
                        yaxis=dict(title='Type de Véhicule'),
                        title_font_size=18,
                        xaxis_title_font_size=14,
                        yaxis_title_font_size=14,
                        coloraxis_colorbar=dict(title="Pourcentage (%)")
        ...
        if "Point de choc" in variables_vehicule:
            st.write("##### Point de choc")
                        # --- 1. Préparation Initiale ---
            data_choc = df_1[['Num_Acc', 'grav_max', 'new_choc']].copy()

            # Gérer les NaN et filtrer la modalité 'Choc_NR'
            data_choc.dropna(subset=['new_choc'], inplace=True)
            data_choc = data_choc[data_choc['new_choc'] != 'Choc_NR']

            if not data_choc.empty:
                # --- 2. Agrégation par Type de choc et Gravité ---
                agg_data = data_choc.groupby(['new_choc', 'grav_max'])['Num_Acc'].nunique().reset_index()
                agg_data.rename(columns={'Num_Acc': 'Nombre_Accidents_Uniques'}, inplace=True)

                # --- 3. Calcul du Total par Type de choc ---
                total_par_choc = data_choc.groupby('new_choc')['Num_Acc'].nunique().reset_index()
                total_par_choc.rename(columns={'Num_Acc': 'Total_Accidents_Choc'}, inplace=True)

                # --- 4. Calcul des Pourcentages ---
                agg_data = pd.merge(agg_data, total_par_choc, on='new_choc', how='left')
                agg_data['Pourcentage'] = agg_data.apply(
                    lambda row: (row['Nombre_Accidents_Uniques'] / row['Total_Accidents_Choc']) * 100
                    if row['Total_Accidents_Choc'] > 0 else 0,
                    axis=1
                )
                agg_data['Pourcentage'].fillna(0, inplace=True)

                # --- 5. Matrices pour heatmap ---
                heatmap_percentages = agg_data.pivot_table(
                    index='grav_max', columns='new_choc', values='Pourcentage', fill_value=0
                )
                heatmap_counts = agg_data.pivot_table(
                    index='grav_max', columns='new_choc', values='Nombre_Accidents_Uniques', fill_value=0
                )

                # --- 6. Tri des colonnes ---
                total_counts = total_par_choc.set_index('new_choc')['Total_Accidents_Choc']
                sorted_choc_labels = total_counts.sort_values(ascending=False).index.tolist()

                heatmap_percentages = heatmap_percentages.reindex(columns=sorted_choc_labels, fill_value=0)
                heatmap_counts = heatmap_counts.reindex(columns=sorted_choc_labels, fill_value=0)

                # --- 7. customdata pour le hover ---
                total_counts_ordered = total_counts[sorted_choc_labels].values
                num_gravity_levels = len(heatmap_percentages.index)
                totals_array = np.tile(total_counts_ordered, (num_gravity_levels, 1))
                specific_counts_array = heatmap_counts.values
                customdata_combined = np.stack([specific_counts_array, totals_array], axis=-1)

                # --- 8. Création du Heatmap ---
                fig = px.imshow(
                    heatmap_percentages,
                    labels=dict(x="Type de Choc (Trié)", y="Gravité", color="Pourcentage (%)"),
                    title="Répartition (en %) de la Gravité des Accidents par Type de Choc",
                    text_auto=True,
                    aspect="auto",
                    color_continuous_scale='YlGnBu'
                )
                ...
                fig.update_traces(
                    ...
                    texttemplate='%{z:.1f}%',
                    hovertemplate=(
                        "<b>Type de Choc:</b> %{x}<br>"
                        "<b>Gravité:</b> %{y}<br>"
                        "<b>Pourcentage:</b> %{z:.2f}%<br>"
                        "<b>Nb Accidents (Gravité):</b> %{customdata[0]:,.0f}<br>"
                        "<b>Total Accidents (Type de Choc):</b> %{customdata[1]:,.0f}<extra></extra>"
                    )
                )

                fig.update_layout(
                    xaxis=dict(title='Type de Choc (Trié)', type='category'),
                    yaxis=dict(title='Gravité'),
                    title_font_size=18,
                    xaxis_title_font_size=14,
                    yaxis_title_font_size=14,
                ...
                st.warning("Aucune donnée disponible après le filtrage. Vérifiez les colonnes 'new_choc' et 'grav_max'.")
...
# Matrices de corrélation
    if matrices_corrélation_checkbox:
        ...
        if "Corrélation selon les conditions des usagers" in variables_matrices_corrélation:
            ...
            st.write("##### Matrice de corrélation selon les conditions des usagers")
        ...
        if "Corrélation selon les conditions de la route" in variables_matrices_corrélation:
            ...
            st.write("##### Matrice de corrélation selon les conditions de la route")
        ...
        if "Corrélation selon les obstacles et types de chocs" in variables_matrices_corrélation:
            ...
            # Affichage de la matrice de corrélation avec Streamlit
            st.write("##### Matrice de corrélation selon les obstacles et types de chocs")
    ...
    # Elaboration dataframe machine learning
    if Dataframe_machine_learning_checkbox: 
        st.write("### Construction du DataFrame destiné aux modèles de Machine Learning")
        st.dataframe(df_total_final)
        colonnes_surlignees = ['Num_Acc', 'lat', 'long', 'jour', 'mois', 'date', 'an', 'hrmn', 'dep', 'int', 'col', 'tué', 'blessé_léger','indemne', 'blessé_hospitalisé', 'gravité_accident']
        html = ""
        for col in df_total_final.columns:
            if col in colonnes_surlignees:
                html += f"<span style='background-color: yellow; padding:4px; margin-right:10px; border-radius:5px;'>{col}</span>"
            else:
                html += f"<span style='margin-right:10px;'>{col}</span>"

                # Affichage dans Streamlit
        st.markdown("**Les colonnes de ce DataFrame sont les suivantes :**")
        st.markdown(
        f"<div style='overflow-x: auto; white-space: nowrap;'>{html}</div>",
        unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)

        colonnes_a_supprimer =  ['Num_Acc', 'lat', 'long', 'jour', 'mois', 'date', 'an', 'hrmn', 'dep', 'int', 'col', 'tué', 'blessé_léger','indemne', 'blessé_hospitalisé', 'gravité_accident']   
        @st.cache_data
        ...
        df_ml_nettoye = supprimer_colonnes(df_machine_learning, colonnes_a_supprimer)

        # Affichage du DataFrame nettoyé
        st.dataframe(df_ml_nettoye)

        st.write("La plupart des variables sont déjà encodée suite à l'aggrégation des datasets initiaux. Après la séparation du Dataset en ensemble d'entraînement et ensemble de test, nous encodons les variables catégorielles 'agg' et 'atm' à l'aide de OneHotEncoder. Pour la variable horaire, nous définissons une fonction d'encodage basée sur les fonctions sinus et cosinus, projetant ainsi l'heure sur le cercle trigonométrique. Cette représentation cyclique permet de capturer la proximité naturelle entre minuit et 23h59, ce qui ne serait pas possible avec une représentation horaire linéaire. ")
...
<ul>
<li><strong>Approches de rééchantillonnage : Oversampling et undersampling</strong></li>
<li><strong>Approches de pondération des métriques pour optimiser les scores selon les critères d'évaluation définis</strong></li>
</ul>
